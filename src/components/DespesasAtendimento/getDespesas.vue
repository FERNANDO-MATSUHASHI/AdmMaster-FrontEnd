        ...
        <tbody>
            ...
            <tr v-for="despesa in filteredDespesas">
                ...
                <td class="d-flex justify-content-around">
                    <div style="width: 60%;"> {{ obterViatura(despesa.atendimento.viaturaId) }}</div>
                    <div style="width: 40%;"><i style="color: red; cursor: pointer;"
                            @click="abrirViaturaModal(despesa.atendimento.viaturaId)"
                            class="fi fi-rr-light-emergency-on"></i></div>
                </td>
                <td>{{ despesa.tipo }}</td>
            ...
            <form id="modalForm" class="row" @submit.prevent="">
                ...
                <div class="col-md-6">
                    ...
                    <select id="atendimento" class="form-select" required v-model="atendimentoId">
                        <option v-if="dataSourceAtendimentos.length == 0" :value="0" selected disabled>Nenhum Atendimento
                            Disponível</option>
                        <option v-else v-for="atendimento in dataSourceAtendimentos" :value="atendimento.id">Atendimento -
                            Qru: {{ atendimento.qru }}</option>
                    </select>
                </div>
...
export default {
    ...
    methods: {
        getDespesas() {
            axios.get('https://localhost:7255/api/DespesasAtendimento/Gerente/'+localStorage.getItem('gerenteId'), {
                headers: {
        ...
        editarDespesa() {
            ...
            let obj = {
                ...
                gerenteId: localStorage.getItem('gerenteId'),
                valor: this.valor,
                tipo: this.tipo,
